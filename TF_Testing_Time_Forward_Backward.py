...
def conv_op(input_op, name, kh, kw, n_out, dh, dw, p):
    ...
    with tf.name_scope(name) as scope:
        ...
        bias = tf.Variable(tf.constant(0.0, dtype = tf.float32, shape = [n_out], name = scope + 'b'))
...
def fc_op(input_op, name, n_out, p):
    ...
    with tf.name_scope(name) as scope:
        ...
        kernel = tf.get_variable(name=scope + 'w', shape=[n_in, n_out], dtype=tf.float32,
                                 initializer=tf.contrib.layers.xavier_initializer_conv2d())
        conv = tf.matmul(input_op, kernel)
        bias = tf.Variable(tf.constant(0.0, dtype=tf.float32, shape = [n_out], name=scope + 'b'))
        z = tf.nn.bias_add(conv, bias)
        activation = tf.nn.relu(z, name=scope)
        p += [kernel, bias]
...
def inference_op(input_op, keep_prob):
    ...
    reshape = tf.reshape(pool5, shape=[-1, flattened_shape], name = 'reshape')
    ...
    return softmax, predictions, fc8, p

## Define function to compute forward / backward time ##
...
def run_benchmark():
    ...
    with tf.Graph().as_default():
        ...
        img_size = 224
        img = tf.Variable(tf.random_normal(shape=[batch_size, img_size, img_size, 3], dtype=tf.float32, stddev=1e-1))

        keep_prob = tf.placeholder(dtype=tf.float32)
        predictions, softmax, fc8, p = inference_op(img, keep_prob)
        ...
        time_tf_run(sess, grad, {keep_prob: 0.5}, 'Backward')
